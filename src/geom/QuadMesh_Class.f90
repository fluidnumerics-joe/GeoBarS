 ...
 SUBROUTINE Initialize_QuadMesh( myQuadMesh, nNodes, nElems, nEdges, N )
   ...
   INTEGER :: i
      ...
      ALLOCATE( myQuadmesh % nodes(1:nNodes), &
                ...
                myQuadMesh % nodeToElement(1:nNodes,0:maxNodalValence,1:2) )
   ...
   INTEGER :: nID
      ...
      DO iEl = 1, nEls
         DO k = 1, 4 
            ...
            myQuadMesh % nodeToElement(nID,0,1) = myQuadMesh % nodeToElement(nID,0,1) + 1
            i = myQuadMesh % nodeToElement(nID,0,1)
            ! And here we add the element ID for the node-to-element connectivity list
            myQuadMesh % nodeToElement(nID,i,1) = iEl
            ! And here we add the local node ID for the node-to-element connectivity list
            myQuadMesh % nodeToElement(nID,i,2) = k
         ENDDO 
      ENDDO 
