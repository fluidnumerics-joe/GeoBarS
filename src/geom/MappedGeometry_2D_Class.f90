  ...
  SUBROUTINE GenerateBoundaryMetrics_MappedGeometry_2D( myGeom, interp  )
   ...
   REAL(prec) :: node(1:2)
      ...
      DO iS = 0, N
         ...
         myGeom % nHat(1:nDims,iS,south)= -signJ*(/ -cv(2,1), cv(1,1) /)
         ...
         myGeom % nHat(1:nDims,iS,west) = -signJ*(/ cv(2,2), -cv(1,2) /)
         ...
         signJ = abs(J)/J                    
         ! Setting southern boundary normal         -dyds    ,    dxds
         myGeom % nHat(1:nDims,iS,north) = signJ*(/ -cv(2,1), cv(1,1) /)
         ...
         myGeom % nHat(1:nDims,iS,east) = signJ*(/ cv(2,2), -cv(1,2) /)
