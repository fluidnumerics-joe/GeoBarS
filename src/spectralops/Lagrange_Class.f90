...
!>@}
  FUNCTION Interpolate_3D_Lagrange( myPoly, f, sE ) RESULT( interpF )  
   ...
   REAL(prec) :: f2d(0:myPoly % N, 0:myPoly % N), f2dInt(0:myPoly % Nc)
   REAL(prec) :: fInt(0:myPoly % Nc, 0:myPoly % N)
   INTEGER    :: N, i, j, k, e
      ...
      DO k = 0, N
         ...
         ENDDO
      ENDDO
      ! Interpolate onto the s=const plane
      f2dInt = MATMUL( fInt, ls )
      DO k = 0, N
         DO j = 0, N
            e = j + (N+1)*k
            f2d(k,j) = f2dInt(e)
         ENDDO
      ENDDO
      ! Interpolate onto the s=const, p=const line
      f1d = MATMUL( f2d, lp )
      ! Interpolate onto the s=const, p=const, q=const point
      interpF = DOT_PRODUCT( f1d, lq )
...
!>@}
 FUNCTION ApplyInterpolationMatrix_3D_Lagrange( myPoly, f ) RESULT( fNew )  
   ...
   ! Local
   REAL(prec) :: f1(0:myPoly % N, 0:myPoly % N)
   REAL(prec) :: fInt1(0:myPoly % M,0:myPoly % N)
   REAL(prec) :: fInt2(0:myPoly % M,0:myPoly % M, 0:myPoly % N) 
   REAL(prec) :: fMapped(0:myPoly % Mc, 0:myPoly % N)
   REAL(prec) :: fInt3(0:myPoly % Mc, 0:myPoly % M)
   INTEGER    :: i, j, k, N, nNew, row
      ...
      DO k = 0, N
         f1   = f(0:N,0:N,k)
         fInt1 = MATMUL( myPoly % Ts, f1 )
         fInt2(0:N,0:N,k) = MATMUL( fInt1, myPoly % Tp )
      ENDDO

      DO j = 0, nNew
         DO i = 0, nNew
            row = i + j*(nNew + 1)
            fMapped(row,0:N) = fInt2(i,j,0:N)
         ENDDO
      ENDDO

      fInt3 = MATMUL( fMapped, myPoly % Tp )

      DO j = 0, nNew
         DO i = 0, nNew
            row = i + j*(nNew+1)
            fNew(i,j,0:nNew) = fInt3(row,0:nNew)
         ENDDO
      ENDDO
...
!>@}
 FUNCTION ApplyDerivativeMatrix_3D_Lagrange( myPoly, f ) RESULT( derF )  
   ...
   ! LOCAL
   REAL(prec) :: floc1(0:myPoly % N, 0:myPoly % Nc)
   REAL(prec) :: floc2(0:myPoly % N, 0:myPoly % Nc)
   REAL(prec) :: floc3(0:myPoly % N, 0:myPoly % Nc)
   REAL(prec) :: dF1(0:myPoly % N, 0:myPoly % Nc)
   REAL(prec) :: dF2(0:myPoly % N, 0:myPoly % Nc)
   REAL(prec) :: dF3(0:myPoly % N, 0:myPoly % Nc)
   INTEGER    :: j, k, col, N
      ...
      dF1 = MATMUL( myPoly % D, floc1 )
      dF2 = MATMUL( myPoly % D, floc2 )
      dF3 = MATMUL( myPoly % D, floc3 )
